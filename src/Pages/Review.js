...
export default class Review extends React.Component{
    ...
    format = () => {
         let list = this.list.map((item) => {
             return <tr onClick={() => window.location.href="/item_review?id=" + item.id} class="item_tr"><td>{item.name}</td><td>{item.subject}</td><td>{item.status}</td></tr>;
        })
    ...
    render(){
        ...
        return(
            <div style={styles.wrapper}>
                <p style={styles.header}>Items To Review</p>
